...
func searchEmployees(w http.ResponseWriter, r *http.Request) {
	...
	if exists {
		...
		if err != nil {
			...
			json.NewEncoder(w).Encode(ServiceError{Message: "Invalid 'id' parameter"})
			return
		}
		...
		if employees != nil {
			...
			json.NewEncoder(w).Encode(ServiceError{Message: "Employee not found"})
			return
		}
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(ServiceError{Message: "Internal Server Error"})
		return
	}
