...
func updateEmployeeDetails(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(ServiceError{Message: "invalid employee id"})
		return
	}
	...
	if err := decoder.Decode(&updatedEmployee); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(ServiceError{Message: "Failed To Decode Json"})
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(ServiceError{Message: msg})
		logs("Failed TO Update Employee Details")
		return
	}

	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(ServiceError{Message: "Employee Updated Succesfully"})
